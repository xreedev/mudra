...
`delivery.json` is copied from the snapshot trainer into `src/assets` and bundled with the React Native application. `recognizeLandmarks(landmarks, BUNDLED_GESTURE_TEMPLATES)` performs the same normalization, feature extraction, nearest-template matching, and `UNKNOWN` thresholding as the browser test module.
