...
export async function load(params: LoadParams): Promise<LoadResult> {
  try {
    ...
    ctx = await initLlama({
      ...
      // n_threads_batch isn't in this version's published ContextParams
      // type, but llama.rn's own native param-forwarding code
      // (node_modules/llama.rn/src/index.ts) reads `parsed.n_threads_batch`
      // — a stale/incomplete type, not a removed field. Verified on real
      // hardware (llm-testbed): ~25-30% faster first-token latency.
